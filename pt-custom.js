...
function estimate() {
	...
	if ($('#dtPairsLogs thead').length > 0) {
		$('#dtPairsLogs tbody tr').each(function() {
			$(this).find('b').remove();
			var difference = ($(this).find('td.bought-cost:last .current-value').text().replace($currency, "").replace(",", "") - $(this).find('td.bought-cost:last .bought-cost').text().replace($currency, "").replace(",", "")).toFixed(2);
			...
			if (difference2 > 0) {
				$(this).find('td.bought-cost:last .bought-cost').append('<b style="color:#05b16f;border-top: 1px solid;"><br>' + $currency + difference.toLocaleString() + '</b>');
				...
				$(this).find('td.bought-cost:last .bought-cost').append('<b style="color:#d85353;border-top: 1px solid;"><br>'+ $currency + difference.toLocaleString() + '</b>');
	...
	if ($('#dtDcaLogs thead').length > 0) {
		$('#dtDcaLogs tbody tr').each(function() {
			...
			if (difference2 > 0) {
				...
				$(this).find('td.current-value:last').append('<b style="color:#d85353;border-top: 1px solid;"><br>' + $currency + difference + '</b>');
	...
	if ($('#dtDustLogs thead').length > 0) {
		$('#dtDustLogs tbody tr').each(function() {
			...
			if (difference2 > 0) {
				...
				$(this).find('td.bought-cost .bought-cost').append('<b style="color:#d85353;border-top: 1px solid;"><br>' + $currency + difference + '</b>');
